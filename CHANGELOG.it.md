...
- Hardening dell'immagine Docker dopo una segnalazione di Trivy: il toolchain di packaging dell'interprete di sistema (pip, setuptools, `pkg_resources`, wheel bundled) viene rimosso a build conclusa, e `uv sync` non lascia più la sua cache di build in `/root/.cache/uv`. Entrambi contenevano copie di pacchetti irraggiungibili dall'app ma segnalate HIGH — setuptools 70.3.0 (CVE-2025-47273) e msgpack 1.1.2 (GHSA-6v7p-g79w-8964). L'app non ne risente: uv installa tutto in `/app/.venv` e l'entrypoint passa da `uv run`. Immagine più leggera come effetto collaterale
