...
- Immagine Docker alleggerita e irrobustita: il toolchain di packaging dell'interprete di sistema (pip, setuptools, `pkg_resources`, wheel bundled) viene rimosso a build conclusa, e `uv sync` non lascia più la sua cache di build in `/root/.cache/uv`. Nessuno dei due è raggiungibile dall'app — uv installa tutto in `/app/.venv` e l'entrypoint passa da `uv run`. Nota: questo **non** ha chiuso le due segnalazioni HIGH che Trivy riporta sull'immagine (setuptools 70.3.0 / CVE-2025-47273 e msgpack 1.1.2 / GHSA-6v7p-g79w-8964); la loro posizione nell'immagine resta ignota e il gate di sicurezza sul container resta rosso per questa release
